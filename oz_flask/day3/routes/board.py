...
class BoardResource(MethodView):
    ...
    def put(self,board_id):
        board = Board.query.get_or_404(board_id) # 아이디 있으면 값 가져오기, 없으면 404 not found 에러
        data = request.json # 유저 입력 데이터 json 형식으로 가져오기
        
        board.title = data['title'] # board title 값을 유저가 입력한 title 값으로 덮어쓰기
        board.content = data['content']
        
        db.session.commit() # db 에 전달하기 alchemy 메소드임
